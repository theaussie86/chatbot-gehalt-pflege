...
For the email field: The table likely has no `email` column yet. We need to add it. Use a Supabase migration or direct SQL. However, since the chat API already uses the admin client (`getSupabaseAdmin()`), we can add the column via the Supabase dashboard or a migration SQL file. For now, the server action should query the existing columns and handle `email` being null gracefully. Plan 03 (email export) will handle saving the email.
...
IMPORTANT: The Supabase client from `createClient()` uses the authenticated user's session. The `salary_inquiries` table may not have RLS policies yet (the chat route uses `getSupabaseAdmin()` which bypasses RLS). For the admin dashboard to work, we need one of:
- Option A: RLS policy allowing authenticated users to read all inquiries (admin-only table)
- Option B: Use the service role client

Since the admin layout already checks `supabase.auth.getUser()` and redirects unauthenticated users, use the regular auth client. If the table has no RLS policy, we'll need to add one. Add a comment in the server action noting this:

```typescript
// NOTE: salary_inquiries needs RLS policy: authenticated users can SELECT all rows.
// If query returns empty despite data existing, check RLS policies.
// To add: CREATE POLICY "allow_authenticated_read" ON salary_inquiries FOR SELECT TO authenticated USING (true);
```

Also check if the table needs an `email` column added. Add a SQL comment at the top of the file:
```typescript
// DB SETUP REQUIRED (if not already done):
...
4. Returns typed `InquiryRow[]`
  ...
  <done>
Server action `getInquiries` fetches salary inquiries with date range and tariff filters. Returns typed InquiryRow array with structured details. Auth-protected via Supabase client. SQL comments document required DB setup.
  </done>
</task>
