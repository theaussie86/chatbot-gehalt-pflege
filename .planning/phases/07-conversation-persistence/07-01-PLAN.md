...
must_haves:
  truths:
    ...
    - "Fresh conversation without stored data starts with formState at section job_details so state machine activates on first API call"
  artifacts:
  ...
  key_links:
    ...
    - from: "apps/web/App.tsx"
      to: "sendMessageToGemini"
      via: "Destructures { text, formState } from response, passes text string to parseResponse"
      pattern: "const \\{ text.*formState \\}"
---
...
Also add a constant for the default initial formState that the state machine expects:
...
- Currently returns just a string (`return data.text || ""`). Change to return the full object: `return { text: data.text || "", formState: data.formState }`.
- Update the error catch to also return the object shape: `return { text: '[PROGRESS: 0] Entschuldigung...' }` instead of a bare string.

Update `apps/web/App.tsx`:
- Add state: `const [formState, setFormState] = useState<FormState>(DEFAULT_FORM_STATE);`
  CRITICAL: Initialize to DEFAULT_FORM_STATE, NOT null. Without a valid initial formState, the first API call never sends currentFormState, the API's state machine branch never activates, and the StepBar is stuck on 'Jobdaten' forever. The default formState with section 'job_details' and the full missingFields list tells the API to enter the state machine branch immediately.
- On mount (useEffect), call `ConversationStore.load()`. If data exists:
  ...
  - If NO stored data exists, formState stays at DEFAULT_FORM_STATE (already initialized above)
- After each successful API response in `handleSendMessage`:
  - Destructure the response from sendMessageToGemini correctly:
    ```typescript
    const { text: rawText, formState: newFormState } = await sendMessageToGemini(textToSend, messages, formState);
    ```
  - Pass `rawText` (the string) to `parseResponse()`. Do NOT pass the whole response object -- parseResponse expects a string argument.
    ```typescript
    const { cleanText, newProgress, resultData, options } = parseResponse(rawText);
    ```
  - Update formState from the API response if provided: `if (newFormState) setFormState(newFormState);`
  - Call `ConversationStore.save({ messages: updatedMessages, formState: newFormState || formState, progress: newProgress ?? progress, updatedAt: new Date().toISOString() })`
...
- Update `handleReset`:
  ...
  - Reset all state to initial values (initial message, DEFAULT_FORM_STATE, 0 progress)
...
3. `App.tsx` initializes formState to DEFAULT_FORM_STATE (not null)
4. `App.tsx` destructures `{ text: rawText, formState }` from sendMessageToGemini and passes rawText string to parseResponse
5. `App.tsx` loads/saves conversation on mount/message
6. `App.tsx` clears storage on completed state
7. "Neues Gesprach" button visible in header
  </verify>
  <done>
Widget compiles. ConversationStore saves/loads/clears conversation data. App.tsx initializes formState to DEFAULT_FORM_STATE so state machine activates on first API call. Response destructuring correctly separates text (string for parseResponse) from formState (object for state tracking). App.tsx auto-resumes on mount, persists after each message, and clears on completion. "Neues Gesprach" button resets without page reload.
  </done>
</task>
...
- Replace the existing ProgressBar usage with StepBar:
  ...
  - Replace with `<StepBar currentSection={formState.section} />`
  - Note: formState is now always a FormState object (never null), initialized to DEFAULT_FORM_STATE, so `formState.section` is safe -- no need for fallback.
...
6. FormState is initialized to DEFAULT_FORM_STATE (section: 'job_details', missingFields populated) so state machine activates on first API call
7. sendMessageToGemini returns { text, formState } and caller destructures correctly, passing rawText string to parseResponse
</verification>
...
- Fresh conversations initialize formState to DEFAULT_FORM_STATE with section 'job_details' and missingFields ['tarif', 'group', 'experience', 'hours', 'state'] so state machine activates immediately
</success_criteria>
