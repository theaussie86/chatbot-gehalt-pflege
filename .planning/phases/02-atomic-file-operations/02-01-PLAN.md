...
must_haves:
  ...
  key_links:
    ...
    - from: "Task 3 (retry UI)"
      to: "Task 1 (structured errors)"
      via: "result.code and result.rolledBack fields"
      pattern: "result\\.rolledBack"
---
...
2. Create a custom error class DocumentUploadError that includes:
   ...
   - rolledBack: boolean (for UI feedback)

3. Modify the rollback section (lines 43-47) to:
   - Wrap storage cleanup in try/catch
   - Throw DocumentUploadError with rolledBack: true after cleanup
...
Do NOT modify any other behavior. Keep storage-first, DB-second pattern.
  ...
  <verify>
1. Create a test file in /tmp with 60MB size:
...
2. Test rollback behavior (ERR-02):
   - Temporarily add `throw new Error('Simulated DB failure')` after successful storage upload but before DB insert
   - Upload a file and verify:
     a. Error response includes `rolledBack: true`
     b. Storage file is cleaned up (check Supabase Storage dashboard)
     c. Console shows rollback log messages
   - Remove the temporary throw statement after testing
  </verify>
  <done>
...
- Rollback behavior verified with simulated DB failure test
  </done>
</task>
...
**CRITICAL DEPENDENCY:** This task depends on Task 1's structured error response format. The uploadDocumentAction MUST return `{ error: string, code: string, rolledBack?: boolean }` for this UI to work correctly. Verify Task 1 is complete before implementing.
...
- After 500ms updates to show error with code and "File removed" (when result.rolledBack is true)
- Retry button is visible and functional
  </verify>
  <done>
- Failed uploads show "Cleaning up..." then "File removed" in toast (when rolledBack: true)
...
3. Rollback visibility:
   - Simulate DB failure (add temporary throw after storage write)
   - Verify toast shows cleanup message with "File removed"
   - Verify storage file was removed (check Supabase dashboard)
   - Verify response includes rolledBack: true
</verification>
