...
must_haves:
  truths:
    ...
    - "Group field validation uses tarif from formState to determine P vs E prefix; if tarif missing, prompts for tarif first"
    - "Retry counts accumulate within session but reset after 30 min inactivity (TTL-based reset)"
  artifacts:
...
export class FieldValidator {
  ...
  private readonly CONTEXT_TTL_MS = 30 * 60 * 1000;  // 30 minutes TTL for session reset
...
**Validation flow:**
1. Get or create context for `{sessionId}:{field}` using getContext() method
   - getContext() checks if existing context has expired (lastUpdated + CONTEXT_TTL_MS < Date.now())
   - If expired, return fresh context with retryCount: 0, lastErrors: [], lastUpdated: now
   - If not expired, return existing context and update lastUpdated timestamp
...
4. For group field: if formState provided and tarif is set, apply P/E prefix logic:
   ...
   - If tarif is NOT yet set in formState, accept the value with a validation error asking for tarif first:
     "Ich brauche erst deinen Tarifvertrag, um die Entgeltgruppe richtig einzuordnen. Arbeitest du im TVöD, TV-L, oder AVR?"
5. On success: reset context (set retryCount to 0), return normalized value
6. On failure: increment retry count, update lastUpdated, format German error, check for near-miss
