...
if ((nextFormState.userIntent === 'data' || nextFormState.userIntent === 'modification') &&
    ...
    try {
        ...
        if (extraction.extracted && Object.keys(extraction.extracted).length > 0) {
            ...
            // Session ID for retry tracking (use projectId or generate)
            const validationSessionId = `${activeProjectId}:${Date.now()}`;
...
**Update modification handler (summary phase, around line 427-521):**

Add FieldValidator to the modification flow as well:

```typescript
// Inside the modification handling block
