...
must_haves:
  truths:
    ...
    - "Tapped chip transforms into sent message bubble with smooth animation"
  artifacts:
...
<task type="auto">
  <name>Task 4: Add chip-to-bubble transform animation</name>
  <files>apps/web/components/SuggestionChips.tsx, apps/web/App.tsx</files>
  <action>
    Implement animated transition where tapped chip transforms into sent message bubble (per CONTEXT.md line 25: "Animated transition: tapped chip transforms into sent message bubble").

    **Approach: FLIP animation technique**

    1. **Capture chip position on tap** (First):
       - Record the tapped chip's bounding rect (getBoundingClientRect)
       - Store in a ref or state: `{ x, y, width, height }`

    2. **Create flying clone** (Invert + Play):
       - When user sends message (not on chip tap, but on actual send with chip text):
       - Create a positioned clone element of the chip
       - Animate it from chip position to message bubble position
       - Use CSS transform for smooth 60fps animation

    **Update SuggestionChips.tsx:**

    Add prop for animation callback:
    ```typescript
    interface SuggestionChipsProps {
      suggestions: string[];
      onSelect: (text: string, chipRef?: DOMRect) => void; // ADD: DOMRect for animation
      isTyping: boolean;
      disabled?: boolean;
    }
    ```

    Capture chip position on click:
    ```typescript
    ...
    **Update App.tsx:**
    ...
    Add state for pending animation:
    ```typescript
    ...
    In handleSendMessage, after adding user message to messages array:
    ```typescript
    // Trigger chip-to-bubble animation if pending
    if (pendingChipAnimation && inputValue === pendingChipAnimation.text) {
    ...
    **Add data attribute to messages container:**
    ```tsx
    <div data-messages-container className="...messages container classes...">
    ```

    **Timing:** Animation duration 300ms with ease-out easing (Claude's discretion per CONTEXT.md).
  </action>
  <verify>
    TypeScript compiles: `cd apps/web && npx tsc --noEmit`
    Build succeeds: `cd apps/web && npm run build`

    Verify animation code exists:
    `grep -n "animateChipToBubble\|pendingChipAnimation" apps/web/App.tsx`
    `grep -n "getBoundingClientRect\|DOMRect" apps/web/components/SuggestionChips.tsx`
  </verify>
  <done>
    Tapped chip animates (transforms/flies) to become the sent message bubble.
    Animation is smooth (CSS transform, ~300ms duration).
    Clone element is cleaned up after animation completes.
  </done>
</task>
...
7. Chip-to-bubble animation code exists and compiles
</verification>
...
- Tapped chip animates to transform into sent message bubble (FLIP animation)
</success_criteria>
