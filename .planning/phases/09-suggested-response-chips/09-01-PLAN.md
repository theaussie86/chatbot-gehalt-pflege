...
must_haves:
  truths:
    ...
    - "Hybrid approach: predefined chips for known fields, AI-generated for open questions"
  artifacts:
    - path: "apps/api/lib/suggestions.ts"
      provides: "SuggestionService class with stage-aware chip generation and AI fallback"
      exports: ["SuggestionService", "generateSuggestions", "generateAISuggestions"]
  ...
  <action>
    ...
    **Stage-specific rules (from CONTEXT.md decisions):**
    ...
    const PREDEFINED_CHIPS: Record<string, string[]> = {
      ...
      // summary stage (confirmation) — per CONTEXT.md line 32
    ...
    **Function signature:**
    ...
    export async function generateSuggestions(
      ...
      //    call generateAISuggestions(lastBotMessage, formState) for open questions
      // 7. Otherwise return empty array
    }

    // AI-generated suggestions for open questions (hybrid approach per CONTEXT.md)
    async function generateAISuggestions(
      botMessage: string,
      formState: FormState
    ): Promise<string[]> {
      // Use lightweight Gemini call to extract 2-4 contextual suggestions
      // from the bot's question. Prompt example:
      // "Given this question: '{botMessage}', suggest 2-4 short German reply options.
      //  Return JSON array of strings. Keep labels under 20 chars."
      // Return empty array on error or timeout (don't block main response)
    }
    ```

    **Note on hybrid approach:** The AI-generated path is for open-ended questions where predefined chips don't exist. It adds latency, so implement with a short timeout (2s) and gracefully fall back to empty array if AI generation fails.
...
<task type="auto">
  ...
  <action>
    ...
    **Finding locations to update:**

    Run: `grep -n "NextResponse.json" apps/api/app/api/chat/route.ts`

    This will list all return points. For each one that includes `formState:`:
    - Add `suggestions: await generateSuggestions(nextFormState, responseText)`

    **Typical patterns to look for:**
    - `return NextResponse.json({ text:` with `formState:` — ADD suggestions
    - `return NextResponse.json({ text:` without `formState:` — SKIP (legacy/error path)

    **For the non-state-machine path (legacy agent path, identifiable by missing formState in response):**
  ...
  <verify>
    ...
    Find all response locations: `grep -n "NextResponse.json" apps/api/app/api/chat/route.ts`
    Verify suggestions added: `grep -c "generateSuggestions" apps/api/app/api/chat/route.ts` should be >= 5
    Verify import exists: `grep "import.*generateSuggestions" apps/api/app/api/chat/route.ts`
  </verify>
  <done>
