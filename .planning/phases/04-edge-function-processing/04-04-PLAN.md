...
must_haves:
  truths:
    ...
    - "Database webhook trigger is configured and active"
  artifacts:
...
<task type="auto">
  <name>Task 2: Verify database webhook trigger configuration</name>
  <files></files>
  <action>
Before testing, verify the database webhook trigger is properly configured.

**Step 1: Check if webhook trigger exists**

Use Supabase CLI or dashboard to verify the trigger:
```bash
# List database webhooks (if using Supabase CLI)
supabase functions list

# Or check via SQL in Supabase dashboard:
# SELECT * FROM supabase_functions.hooks WHERE name LIKE '%embed%';
```

**Step 2: Document the expected trigger configuration**

The webhook trigger should be configured to:
- Fire on INSERT to the `documents` table
- Call the `process-embeddings` edge function
- Pass the new row data in the request body

**Step 3: If trigger doesn't exist, create it**

In Supabase Dashboard > Database > Webhooks, create:
- Name: `process-embeddings-on-insert`
- Table: `documents`
- Events: `INSERT`
- URL: Edge function URL (get from `supabase functions list`)
- HTTP Method: POST
...
```bash
supabase functions list
# Should show process-embeddings function
```
...
```
  ...
  <verify>
- Confirm webhook trigger exists for documents INSERT
- Confirm edge function is deployed and accessible
- Log the trigger configuration for the checkpoint verification
  </verify>
  <done>
- Database webhook trigger is confirmed active
- Edge function deployment is verified
- Configuration is documented for manual verification
  </done>
</task>
...
1. Confirm the edge function is deployed: Run `supabase functions list` and verify `process-embeddings` appears
2. Confirm the database webhook trigger is configured (check Supabase Dashboard > Database > Webhooks)
3. Confirm the migration ran (check that documents table has `processing_stage` and `chunk_count` columns)
