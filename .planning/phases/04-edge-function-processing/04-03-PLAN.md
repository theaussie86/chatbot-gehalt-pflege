...
1. **Update chunking parameters** to match CONTEXT.md:
```typescript
...
2. **Add content validation** after text extraction:
```typescript
...
3. **Add minimum chunk validation**:
```typescript
...
1. **Add supported MIME types constant**:
```typescript
...
2. **Add MIME type validation** at the start of processing:
```typescript
...
3. **Add extraction prompt customization** based on file type:
```typescript
...
5. **Add image-only PDF detection** (heuristic based on extracted text):
```typescript
const textContent = extractResult.text || "";
...
- Confirm image-only PDF detection using bytes-per-char heuristic
  </verify>
  <done>
...
1. **Code review**: Verify:
   ...
   - Spreadsheet-specific extraction prompt
